...
class DatadogServiceFactory(object):
  ...
  def add_argparser(self, parser):
    ...
    parser.add_argument('--datadog_host', default='', required=False,
                        help='Specify the host to report back to datadog with.'
                        ' The default will be the hostname in the --dd_agent_config file'
                        ' or localhost if not found. This name will be resolved to a fqdn.')
    parser.add_argument(
        '--dd_agent_config', default='/etc/dd-agent/datadog.conf',
        help='Path to datadog agent config file (to read keys from).'
        ' This is not user-readable so you may need to use environment variables for'
        ' DATADOG_API_KEY and DATADOG_APP_KEY instead.')

  def read_param(self, param_name, config_text):
    """Read configuration parameter from Datadog config_text."""
    match = re.search('^{0}:(.*)$'.format(param_name), config_text, re.MULTILINE)
    if not match:
      print 'NO {0}'.format(param_name)
      return None
    print 'FOUND {0}'.format(param_name)
  ...
  def __call__(self, options, command_handlers):
    ...
    try:
      with open(config_path, 'r') as stream:
        ...
        text = stream.read()
        app_key = self.read_param('app_key', text)
        api_key = self.read_param('api_key', text)
        host = self.read_param('hostname', text)
